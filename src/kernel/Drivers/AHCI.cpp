...
namespace AHCI{
    ...
    void AHCIDriver::ProbePorts(){
        ...
        for (int i = 0; i < 32; i++){
            if (portsImplemented & (1 << i)){
                ...
                {
                    DebugPrint("SATA drive");
                }
                else if (portType == PortType::SATAPI) 
                {
                    DebugPrint("SATAPI drive");
                }else 
                {
    ...
    AHCIDriver::AHCIDriver(PCI::PCIDeviceHeader* pciBaseAddress){
        ...
        DebugPrint("AHCI Driver instance initialized");
        PCI::PCIHeader0 Header = {
            *pciBaseAddress
        };
        uintptr_t _unusedPointer = Header.BAR5;

        ABAR = (HBAMemory*)_unusedPointer;
